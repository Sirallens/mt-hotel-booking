...
>>>>>>> 79cef2eb72c241cee5b44b906bd29afe434539b2
        wp_localize_script(
            ...
            array(
                ...
                'room_types' => $room_types_for_js, // Enhanced with occupancy parameters
    ...
    public function handle_booking()
    {
        ...
        // NEW: Occupancy validation using HBS_Occupancy_Validator
        $occupancy_validation = HBS_Occupancy_Validator::validate($room_type, $adults_count, $kids_count);

        if (!$occupancy_validation['valid']) {
            wp_send_json_error(
                array(
                    'msg' => $occupancy_validation['error'],
                )
            );
        }
